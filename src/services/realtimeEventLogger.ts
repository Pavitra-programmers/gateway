...
async function getBoss(): Promise<any> {
  ...
  const boss = new PgBoss({ connectionString, retryLimit: 5, disableMigration: true });
...
export async function realtimeEventLogger(
  ...
  try {
    ...
    if (eventType === 'response.done' && eventData.response) {
      const response = eventData.response;
      
      // Extract usage from response - handle multiple provider formats
      if (response.usage) {
        ...
        // Google Gemini Live may use different field names
        if (!inputTokens && response.usage.prompt_token_count) {
          inputTokens = response.usage.prompt_token_count;
        }
        if (!outputTokens && response.usage.candidates_token_count) {
          outputTokens = response.usage.candidates_token_count;
        }
      }
      
      // Extract model from response or session

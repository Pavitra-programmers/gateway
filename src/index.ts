...
// Load conf.json if present; otherwise fallback to conf_sample.json or an empty conf.
let conf: any = {};
try {
  ...
  try {
    const samplePath = new URL('../conf_sample.json', import.meta.url);
    conf = JSON.parse(fs.readFileSync(samplePath, 'utf-8'));
  } catch (err2) {
    conf = {};
  }
}

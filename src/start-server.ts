...
import { realtimeEventLogger } from './services/realtimeEventLogger.js';
...
try {
  ...
  console.log('🔌 MCP Gateway mounted at /mcp');
} catch (error: any) {
  console.warn('⚠️  MCP Gateway not loaded:', error.message);
}
...
// Register realtime event logger for cost tracking
// This middleware sets the realtimeEventParser in context for WebSocket handlers
// IMPORTANT: Must be registered BEFORE the WebSocket routes
app.use('/v1/realtime', async (c, next) => {
  c.set('realtimeEventParser', realtimeEventLogger);
  console.log('[gateway] realtimeEventParser registered for /v1/realtime');
  return next();
});

app.use('/stringcost-ws/*', async (c, next) => {
  c.set('realtimeEventParser', realtimeEventLogger);
  console.log('[gateway] realtimeEventParser registered for /stringcost-ws/*');
  return next();
});

// Original Portkey realtime endpoint (no auth)
app.get(
  '/v1/realtime',
  requestValidator,
...
// StringCost WebSocket proxy with token auth
// Pattern: /stringcost-ws/t/{token}/v1/realtime or any path after token
// The middleware above should set realtimeEventParser before this handler runs
app.get(
